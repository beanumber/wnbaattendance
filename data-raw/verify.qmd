...
mod_attendance <- lm(
  log(attendance) ~ log(arena_capacity) + factor(period) * (home_win_prob) + factor(wday(game_date)) + factor(month(game_date)) + name_current + is_cc + median_income + median_pop,
  data = wnba_gl_reg
)
