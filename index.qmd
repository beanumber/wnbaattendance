...
Using our definition, we deemed `r round(100 * nrow(filter(wnba_gl, is_sellout)) / nrow(wnba_gl), 1)`% of the games as sellouts. 
...
form <- paste("log(attendance + 1) ~ ", f_exp, "* factor(season_id) +", f_place, "+", f_time) |>
  as.formula()
```

```{r}
# OLS model
mod_ols <- lm(formula = form, data = wnba_df)

# Tobit model
library(VGAM)

mod_tobit <- vglm(
  formula = mod_ols$call,
...
mod_pct <- f_exp |>
  ...
  map(as.formula) |>
  map(glm, data = wnba_df, family = binomial(link = "logit")) |>
...
wnba_sun <- wnba_df |>
  filter(name_current == "Connecticut Sun")

ggplot(data = wnba_sun, aes(y = attendance, x = z_quality_of_play, color = is_sellout)) +
  geom_point() +
  facet_wrap(vars(arena))

mod_simple <- lm(
  log(attendance) ~ (I(z_quality_of_play) + I(z_uncertain_outcome) + I(z_loss_aversion)) + factor(season_id) + factor(wday(game_date)) + factor(month(game_date)) + is_cc + is_aw, 
  data = wnba_sun,
  na.action = "na.exclude"
)
...
wnba_sun <- wnba_sun |>
  mutate(y_hat_simple = exp(fitted(mod_simple)))

ggplot(data = wnba_sun, aes(x = attendance, y = y_hat_simple, color = is_sellout)) +
  geom_abline(slope = 1, linetype = 3) + 
  geom_point() +
  facet_wrap(vars(arena))
```
...
mod_tobit <- vglm(
  mod_simple$call,
  family = tobit(Upper = ifelse(wnba_sun$is_sellout, log(wnba_sun$attendance), Inf), imethod = 2), 
  data = wnba_sun,
  na.action = "na.exclude"
)
...
wnba_sun <- wnba_sun |>
  mutate(y_hat_tobit = exp(fitted(mod_tobit)))

ggplot(data = wnba_sun, aes(x = attendance, y = y_hat_tobit, color = is_sellout)) +
  geom_abline(slope = 1, linetype = 3) + 
  geom_point() +
  facet_wrap(vars(arena))

ggplot(data = wnba_sun, aes(y = y_hat_simple, x = y_hat_tobit, color = is_sellout)) +
  geom_abline(slope = 1, linetype = 3) + 
  geom_point() +
  facet_wrap(vars(name_current))

ggplot(data = wnba_sun, aes(x = attendance - y_hat_tobit, y = y_hat_tobit, color = is_sellout)) +
  geom_abline(slope = 1, linetype = 3) + 
  geom_point() +
  facet_wrap(vars(arena))
```
